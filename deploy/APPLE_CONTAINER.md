...
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.004
```
...
docker build \
  ...
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.004 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.183-custom.004`. Until then, keep
