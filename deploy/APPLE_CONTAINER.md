...
```dotenv
APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luohao830/sub2api-plus:latest
...
Apple/OCI image:    ghcr.io/luohao830/sub2api-plus:v0.1.183-custom.004
```
...
```bash
docker build \
  --build-arg VERSION=0.1.183+custom.004 \
  --tag ghcr.io/luohao830/sub2api-plus:v0.1.183-custom.004 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luohao830/sub2api-plus:v0.1.183-custom.004`. Until then, keep
...
SUB2API_IMAGE=ghcr.io/luohao830/sub2api-plus:latest # Match APPLE_CONTAINER_SUB2API_IMAGE in .env.
