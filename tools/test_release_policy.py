...
class PublishedReleaseCheckTests(unittest.TestCase):
    def test_ci_wrapper_verifies_remote_tag_workflow_and_assets_read_only(self) -> None:
        ...
        remote_tag.assert_called_once_with("luohao830/sub2api-plus", TAG)
        workflow.assert_called_once_with(
            "luohao830/sub2api-plus",
            TAG,
            OFFICIAL_COMMIT,
        )
        release.assert_called_once_with("luohao830/sub2api-plus", TAG)
...
class ReleaseDocumentTests(unittest.TestCase):
    ...
    def document_text(rule: release_docs.DocumentRule) -> str:
        ...
        current_value_fixtures = {
            "deploy/README.md": (
                f"Git/GitHub: {old}",
                f"GHCR: ghcr.io/luohao830/sub2api-plus:{old_oci}",
            ),
            "deploy/DOCKER.md": (
            ...
            "deploy/APPLE_CONTAINER.md": (
                ...
                f"Apple/OCI image: ghcr.io/luohao830/sub2api-plus:{old_oci}",
                f"--build-arg VERSION={old_application} \\",
                f"--tag ghcr.io/luohao830/sub2api-plus:{old_oci} \\",
                f"APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luohao830/sub2api-plus:{old_oci}",
            ),
            "deploy/.env.example": (
